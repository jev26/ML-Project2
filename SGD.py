...
def matrix_factorization_SGD(ratings):
    ...
    while ((errors[-2] - errors[-1]) > stop_criterion):
        ...
        for d, n in nz_ratings:
            # Update of the user_ and item_ features matrices
            e = ratings[d, n] - prediction(user_features[:, n], item_features[:, d])
            user_features[:, n] += gamma * (e * item_features[:, d] - lambda_user * user_features[:,
                                                                                    n])  # Update latent user feature matrix
        ...
        print("iter: {}, RMSE :{}.".format(it, rmse))
